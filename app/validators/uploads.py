        ...
        for fileStorageObject in files:
            ...
            if extension in TABULAR_DATA_EXTENSIONS:
                ...
                file_path = os.path.join(
                    os.getenv("TABULAR_FILESTORAGE", "/media/tabular"),
                    filename_with_extension,
                )
            elif extension in TEXTUAL_DATA_EXTENSIONS:
                file_type = "textual"
                file_path = os.path.join(
                    os.getenv("TEXTUAL_FILESTORAGE", "/media/textual"),
                    filename_with_extension,
                )
            elif extension in PICTURE_EXTENSIONS:
                file_type = "rgb"
                file_path = os.path.join(
                    os.getenv("RGB_FILESTORAGE", "/media/rgb"),
                    filename_with_extension,
                )
            else:
            ...
            if FilesModel.get_file_by_name_user(user_id, file_name):
                raise ValidationError(
                    f"{filename_with_extension} file already exists, "
                    f"please remove it from uploads."
                )
